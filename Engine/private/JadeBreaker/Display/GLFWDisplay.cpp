...
namespace Sherphy 
{
	void GLFWDisplay::init(uint32_t width, uint32_t height) 
	{
		...
		SHPY_EXCEPTION_IF_FALSE(m_window, "GLFW Display Window Init Faild\n");
	}
	...
	{
		SHPY_EXCEPTION_IF_FALSE(glfwCreateWindowSurface(instance, m_window, allocator, surface) == VK_SUCCESS, "failed to create window surface!");
	}
