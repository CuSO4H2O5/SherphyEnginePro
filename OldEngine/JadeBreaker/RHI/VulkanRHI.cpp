...
namespace Sherphy{
    static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData) {
        SHPY_RENDERING_LOG(pCallbackData->pMessage);
        return VK_FALSE;
    }
    ...
    void VulkanRHI::createInstance()
    {
        ...
        {
            SHPY_EXCEPTION_IF_FALSE(checkValidationLayerSupport(), "validation layers requested, but not avaliable.");
        }
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateInstance(&create_info, nullptr, &m_instance) == VK_SUCCESS, "create vk instance failed!\n");
    ...
    void VulkanRHI::setupDebugMessenger() {
        SHPY_RETURN_IF_FALSE(m_enable_validation_layer, "No Validation Layer Debug Info");

        VkDebugUtilsMessengerCreateInfoEXT createInfo;
        populateDebugMessengerCreateInfo(createInfo);

        SHPY_EXCEPTION_IF_FALSE(CreateDebugUtilsMessengerEXT(m_instance, &createInfo, nullptr, &m_debug_messenger) == VK_SUCCESS, "failed to set up debug messenger!");
    }
    ...
    void VulkanRHI::createDescriptorSetsNormal() 
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkAllocateDescriptorSets(m_device.m_logical_device, &alloc_info, m_descriptor_sets.data()) == VK_SUCCESS, "failed to allocate descriptor sets!");
    ...
    void VulkanRHI::createDescriptorPoolNormal() 
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateDescriptorPool(m_device.m_logical_device, &pool_info, nullptr, &m_descriptor_pool) == VK_SUCCESS, "failed to create descriptor pool!");
    }
    ...
    void VulkanRHI::createUniformBuffers()
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            SHPY_ASSERT(m_device.createBuffer(buffer_size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
    ...
    void VulkanRHI::updateUniformBuffer(uint32_t current_image)
    {
        ...
        SHPY_MEMCPY(m_uniform_buffers[current_image].mapped, &ubo, sizeof(ubo));
    }
    ...
    void VulkanRHI::createDescriptorSetLayoutRayTracing()
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateDescriptorSetLayout(m_device.m_logical_device, &create_info, nullptr, &m_descriptor_set_layout) == VK_SUCCESS, "RayTracing Descriptor Layout Creation Failed");
    }
    ...
    void VulkanRHI::createDescriptorSetLayoutNormal() 
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateDescriptorSetLayout(m_device.m_logical_device, &layout_info, nullptr, &m_descriptor_set_layout) == VK_SUCCESS,
    ...
    {
        SHPY_EXCEPTION_IF_FALSE(m_vertices.size() != 0, "no vertices input\n");
        VkDeviceSize buffer_size = sizeof(m_vertices[0]) * m_vertices.size();
        switch (type)
        {
        case Sherphy::PipeLineType::RayTracing:
            SHPY_ASSERT(m_device.createBuffer(buffer_size,
        ...
        default:
            ...
            SHPY_ASSERT(m_device.createBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                staging_buffer, m_vertices.data()), VK_SUCCESS, "");

            SHPY_ASSERT(m_device.createBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
    ...
    void VulkanRHI::createIndexBuffer(PipeLineType type)
    {
        ...
        default:
            VulkanBuffer staging_buffer;
            SHPY_ASSERT(m_device.createBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, staging_buffer, m_indices.data()), VK_SUCCESS, "");

            SHPY_ASSERT(m_device.createBuffer(buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_index_buffer), VK_SUCCESS, "");
    ...
    void VulkanRHI::createTransformBuffer(PipeLineType type)
    {
        ...
        {
        case Sherphy::PipeLineType::RayTracing:
            SHPY_ASSERT(m_device.createBuffer(sizeof(VkTransformMatrixKHR),
                VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR,
                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
    ...
    void VulkanRHI::recordCommandBuffer(VkCommandBuffer command_buffer, uint32_t image_index) {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkBeginCommandBuffer(command_buffer, &begin_info) == VK_SUCCESS, "failed to begin recording command buffer!");
        ...
        SHPY_EXCEPTION_IF_FALSE(vkEndCommandBuffer(command_buffer) == VK_SUCCESS, "failed to record command buffer!");
    }
    ...
    VkFormat VulkanRHI::findSupportedFormat(VkPhysicalDevice device, const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
        ...
        SHPY_EXCEPTION_IF_FALSE(false, "failed to find supported format!");
    }
    ...
    void VulkanRHI::createTextureImage() 
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(pixels, "failed to load texture image!");

        VulkanBuffer staging_buffer;
        SHPY_ASSERT(m_device.createBuffer(image_size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, staging_buffer), VK_SUCCESS, "");

        void* data;
        vkMapMemory(m_device.m_logical_device, staging_buffer.memory, 0, image_size, 0, &data);
        SHPY_MEMCPY(data, pixels, static_cast<size_t>(image_size));
    ...
    VkImageView VulkanRHI::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspect_flags)
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateImageView(m_device.m_logical_device, &view_info, nullptr, &image_view) == VK_SUCCESS, "failed to create texture image view!");
    ...
    void VulkanRHI::createTextureSampler()
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateSampler(m_device.m_logical_device, &sampler_info, nullptr, &m_sampler) == VK_SUCCESS, "failed to create texture sampler!");
    }
                                ...
                                VkDeviceMemory& image_memory) 
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateImage(m_device.m_logical_device, &image_info, nullptr, &image) == VK_SUCCESS, "failed to create image!");
        ...
        SHPY_EXCEPTION_IF_FALSE(vkAllocateMemory(m_device.m_logical_device, &alloc_info, nullptr, &image_memory) == VK_SUCCESS, "failed to allocate image memory!")
    ...
    void VulkanRHI::createFrameBuffers() 
    {
        ...
        for (size_t i = 0; i < m_swap_chain_image_views.size(); i++) {
            ...
            SHPY_EXCEPTION_IF_FALSE(vkCreateFramebuffer(m_device.m_logical_device, &frame_buffer_info, nullptr, &m_swap_chain_frame_buffers[i]) == VK_SUCCESS, "failed to create framebuffer!");
        }
    }
    ...
    void VulkanRHI::pickPhysicalDevice()
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(device_count > 0, "Failed to Find Graphic Device with Vulkan Support\n");

        for (uint32_t device_id = 0; device_id < device_count; device_id++)
        {
            VkPhysicalDevice& device = m_physical_devices[device_id];
            SHPY_CONTINUE_WITH_LOG(device, "WTF");
            if (isDeviceSuitable(device))
            {
                VkPhysicalDeviceProperties prop;
                vkGetPhysicalDeviceProperties(device, &prop);
                SHPY_LOG(prop.deviceName);
    ...
    void VulkanRHI::createSwapChain(VkPhysicalDevice device)
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateSwapchainKHR(m_device.m_logical_device, &create_info, nullptr, &m_swap_chain) == VK_SUCCESS, "failed to create swap chain!");
    ...
    {
        SHPY_EXCEPTION_IF_FALSE((m_swap_chain_images.size()>0), "swap chain image is empty");
    ...
    VkShaderModule VulkanRHI::createShaderModule(const std::vector<char>& code) 
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateShaderModule(m_device.m_logical_device, &create_info, nullptr, &shader_module) == VK_SUCCESS, "failed to create shader module!");
    ...
    void VulkanRHI::createRenderPassNormal() 
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateRenderPass(m_device.m_logical_device, &render_pass_info, nullptr, &m_render_pass) == VK_SUCCESS, "failed to create render pass!");
        return;
    }
    ...
    void VulkanRHI::createTopLevelAccelerationStructure() 
    {
        ...
        SHPY_ASSERT(m_device.createBuffer(sizeof(VkAccelerationStructureInstanceKHR),
            VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        ...
        */
        ...
        SHPY_ASSERT(m_device.createBuffer(acceleration_structure_build_sizes_info.buildScratchSize,
    ...
    void VulkanRHI::createAccelerationStructureBuffer(AccelerationStructure& acceleration_structure, VkAccelerationStructureBuildSizesInfoKHR build_size_info)
    {
        ...
        SHPY_ASSERT(vkCreateBuffer(m_device.m_logical_device, &buffer_create_info, nullptr, &acceleration_structure.buffer), VK_SUCCESS, "AccelerationStructureBuffer Faild");
        ...
        SHPY_ASSERT(vkAllocateMemory(m_device.m_logical_device, &memory_allocate_info, nullptr, &acceleration_structure.memory), VK_SUCCESS, "AccelerationStructureBuffer AllocateMemory");
        SHPY_ASSERT(vkBindBufferMemory(m_device.m_logical_device, acceleration_structure.buffer, acceleration_structure.memory, 0), VK_SUCCESS, "AccelerationStructureBuffer BindBufferMemory");
    }
                                                     ...
                                                     const std::vector<char>& closest_hit_shader)
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreatePipelineLayout(m_device.m_logical_device, &pipeline_layout_create_info, nullptr, &m_pipeline_layout), "");
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateRayTracingPipelinesKHR(m_device.m_logical_device, VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &rayTracing_pipeline_CI, nullptr, &m_graphics_pipeline) == VK_SUCCESS, "create RayTracingPipeline Faild");
    }
                                                  ...
                                                  const std::vector<char>& fragment_shader)
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreatePipelineLayout(m_device.m_logical_device, &pipeline_layout_info, nullptr, &m_pipeline_layout) == VK_SUCCESS, "");
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateGraphicsPipelines(m_device.m_logical_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &m_graphics_pipeline) == VK_SUCCESS, "failed to create graphics pipeline!");
    ...
    void VulkanRHI::createSyncObjects() 
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            SHPY_EXCEPTION_IF_FALSE(vkCreateSemaphore(m_device.m_logical_device, &semaphore_info, nullptr, &m_image_available_semaphores[i]) == VK_SUCCESS, "failed to create image semaphore for a frame!");
            SHPY_EXCEPTION_IF_FALSE(vkCreateSemaphore(m_device.m_logical_device, &semaphore_info, nullptr, &m_render_finished_semaphores[i]) == VK_SUCCESS, "failed to create render finish semaphore for a frame!");
            SHPY_EXCEPTION_IF_FALSE(vkCreateFence(m_device.m_logical_device, &fence_info, nullptr, &m_in_flight_fences[i]) == VK_SUCCESS, "failed to create fence for a frame!");
        }
    ...
    void VulkanRHI::drawFrame() 
    {
        ...
        if (result == VK_ERROR_OUT_OF_DATE_KHR) {
            ...
            SHPY_EXCEPTION_IF_FALSE(result == VK_SUCCESS || result == VK_SUBOPTIMAL_KHR, "failed to acquire swap chain image!");
        }
        ...
        SHPY_EXCEPTION_IF_FALSE(vkQueueSubmit(m_graphics_queue, 1, &submit_info, m_in_flight_fences[m_current_frame]) == VK_SUCCESS, "failed to submit draw command buffer!");
        ...
        {
            SHPY_EXCEPTION_IF_FALSE(result == VK_SUCCESS, "failed to present swap chain image!");
        }
