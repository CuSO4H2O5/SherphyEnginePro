...
namespace Sherphy 
{
    void VulkanDevice::createLogicalDevice(VkSurfaceKHR surface, VkPhysicalDeviceFeatures enabled_features, const std::vector<const char*>& enabled_extensions, void* pNext_chain, bool use_swap_chain)
    {
        ...
        SHPY_EXCEPTION_IF_FALSE((vkCreateDevice(m_physical_device, &create_info, nullptr, &m_logical_device) == VK_SUCCESS), "faild to create logical device");
    ...
    QueueFamilyIndices VulkanDevice::findQueueFamilies(VkPhysicalDevice& device, VkSurfaceKHR surface)
    {
        ...
        SHPY_ASSERT(queue_family_count > 0, true, "Device Queue Family Properties is null");
                                    ...
                                    VulkanBuffer& buffer, void* data)
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkCreateBuffer(m_logical_device, &buffer_info, nullptr, &buffer.buffer) == VK_SUCCESS, "failed to create vertex buffer!");
        ...
        SHPY_EXCEPTION_IF_FALSE(vkAllocateMemory(m_logical_device, &alloc_info, nullptr, &buffer.memory) == VK_SUCCESS, "failed to allocate vertex buffer memory!");

        if (data != nullptr)
        {
            SHPY_ASSERT(buffer.map(), VK_SUCCESS, "");
    ...
    void VulkanDevice::createCommandBuffers(uint32_t frame_count)
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(vkAllocateCommandBuffers(m_logical_device, &alloc_info, m_command_buffers.data()) == VK_SUCCESS, "failed to allocate command buffers!");
    }
    ...
    VkCommandPool VulkanDevice::createCommandPool()
    {
        ...
        VkCommandPool pool;
        SHPY_EXCEPTION_IF_FALSE(vkCreateCommandPool(m_logical_device, &pool_info, nullptr, &pool) == VK_SUCCESS, "failed to create command pool!");
        return pool;
    ...
    uint32_t VulkanDevice::findMemoryType(uint32_t type_filter, VkMemoryPropertyFlags properties)
    {
        ...
        SHPY_EXCEPTION_IF_FALSE(false, "failed to find suitable memory type!");
    }

    void VulkanDevice::getPhysicalDeviceProperties()
    {
        SHPY_EXCEPTION_IF_FALSE(m_physical_device, "Did not detected Proper Physical Device");
    ...
    void VulkanDevice::endSingleTimeCommands(VkCommandBuffer command_buffer, VkQueue queue)
    {
        ...
        VkFence fence;
        SHPY_ASSERT(vkCreateFence(m_logical_device, &fence_info, nullptr, &fence), VK_SUCCESS, "Vk Single Time Command Fence Creation Failed");
        SHPY_ASSERT(vkQueueSubmit(queue, 1, &submit_info, fence), VK_SUCCESS, "Queue Submit Failure");
        SHPY_ASSERT(vkWaitForFences(m_logical_device, 1, &fence, VK_TRUE, UINT64_MAX), VK_SUCCESS, "Fence Wait Time Out");
